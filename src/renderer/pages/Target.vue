            ...
            <div>
                <div class="Target-Left">
                    <div style="width:auto;height:45px">
                        <ButtonGroup style="float:left">
                            <Button @click="queryDoingTarget()" style="border-radius:0;width:101px"
                                    :type="theme.doingStyle">Doing
                            </Button>
                    ...
                    <div id="Target-Left-TimeLine" style="width:312px;text-align:left;height:555px;">
                        <div style="width:312px;height:555px" id="Left-Target-List"
                             v-if='theme.doingStyle === "primary"'>
                            <div v-for="(item, index) in theme.doingTargets" :key="index"
                                 style="padding:5px;width:302px;height:50px;display:inline-block;float:left">
                                <img style="width:40px;height:40px;float:left;margin-right:10px;display:inline-block"
                                     src="../assets/img/target.png" alt="">
                                <img @click="showDeleteThemeOrPeriodModal(item.themeId, item.periodId)"
                                     style="width:40px;height:40px;float:right;display:inline-block"
                                ...
                                <span style="width:192px;display:inline-block;margin-right:10px">
                                    <el-tag style="margin-right:10px;color:#fff;background:#2D8CF0" effect="dark"
                                            size='mini'>第{{item.period}}期</el-tag>
                                    <strong style="font-size:12px">还剩&nbsp<span
                                            style="color:red">{{item.remainDays}}</span>&nbsp天</strong>
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="TimeMachine-Right">
                    <div class="TimeMachine-Right-Header">
                        <strong>目标清单</strong>
                        <DatePicker class="TimeMachine-Right-Header-DatePicker" type="date"
                                    v-model="period.curDateTime"></DatePicker>
                    </div>

                    <div class="Target-Steps">
                        <template>
                            <Steps :current=theme.current>
                                <span style="cursor:pointer" @click="nextStep(0)"><Step title="第一步"
                                                                                        content="请写出你的目标"></Step></span>
                                <span style="cursor:pointer" @click="nextStep(1)"><Step title="第二步"
                                                                                        content="分解你的目标吧"></Step></span>
                                <span style="cursor:pointer" @click="nextStep(2)"><Step title="第三步"
                                                                                        content="去完成你的目标吧"></Step></span>
                            </Steps>
                        </template>
                    </div>
                    ...
                    <div id="Target-Step-First" class="Target-Step" style="display:block">
                        <div style="width:auto;height:35px;margin-bottom:10px">
                            <h3 style="line-height:35px;display:inline-block;float:left;margin-right:10px;width:90px;text-align:right">
                                主题: </h3>
                        ...
                        <div style="width:auto;height:35px;">
                            <h3 style="line-height:35px;display:inline-block;float:left;margin-right:10px;">
                                每天可用时间: </h3>
                    ...
                    <div id="Target-Step-Second" style="display:none" class="Target-Step">
                        <div style="display:block;width:620px;height:50px;margin-bottom:10px">
                            <i-button style="float:left;margin-top:10px;width:620px;display:block" type="primary">
                                批量导入子任务
                            </i-button>
                        </div>
                        <div id="Target-Step-Second-ChildTarget" style="width:640px;height:420px;background:#fff">
                            <div style="width:620px;height:10px"></div>
                            <div v-for="(item, index) in target.childTargets" :key="index"
                                 style="margin-bottom:15px;padding-top:10px;border-right: 1px solid #ddd;box-shadow: 0 0 5px #ccc;width:620px;height:96px;">
                                <div style="width:620px;height:34px;margin-bottom:10px;">
                                    <h3 style="line-height:34px;display:inline-block;float:left;margin-right:10px;width:46px;text-align:right">
                                        子目标: </h3>
                                    <Input v-model="item.target" style="width: 546px;float:left;margin-right:10px"/>
                                    <span @click="deleteChildTarget(index)"
                                          style="float:right;margin-top:-52px;margin-right:-6px;cursor:pointer">
                                ...
                                <div style="width:620px;height:34px;display:block">
                                    <h3 style="line-height:34px;display:inline-block;float:left;margin-right:10px;width:46px;text-align:right">
                                        耗时: {{index}}</h3>
                                    <InputNumber v-model="item.consumeTime"
                                                 style="width: 200px;float:left; margin-right:12px">
                                        <span slot="append">hour</span>
                                    </InputNumber>
                    ...
                    <div id="Target-Step-Third" style="display:none" class="Target-Step">
                        ...
                        <div style="width:auto;height:35px;">
                            ...
                            <div style="float:left;">
                                <DatePicker placement="top-start" v-model="period.deadline" type="date" size="default"
                                            format="yyyy-MM-dd" style="width: 510px"></DatePicker>
                            </div>
                        </div>
                        <div style="width:auto;height:35px;margin-bottom:10px">
                            <h3 style="line-height:35px;display:inline-block;float:left;margin-right:10px;width:90px;text-align:right">
                                是否完成: </h3>
                            <RadioGroup v-model="period.isCompleted"
                                        style="float:left;font-size: 22px;font-weight: 500;">
                        ...
                        <div style="margin-bottom:10px; margin-top:20px">
                            ...
                            <Modal id="childTargetModal" :mask-closable="false" v-model="theme.childTargetsModal"
                                   :styles="{top: '300px'}" title="子目标详情!">
                                <div style="width:488px;height:335px;">
                                    <h3 style="line-height:35px; text-align:center;width:488px;font-weight:16px">
                                        子目标</h3>
                                    <div style="width:488px;height:300px" id="Target-ChildTargetModal">
                                        <span style="font-size:14px; display:block; width:488px; font-weight:500;margin-bottom:10px"
                                              v-for="(item, index) in childTargets" :key="index">
                            ...
                            <Modal :mask-closable="false" v-model="theme.deleteTargetOrPeriodModal" title="删除主题或者当前期数"
                                   :styles="{top: '300px', width:'400px', height:'200px'}" @on-ok="deleteThemeOrPeriod"
                                   @on-cancel="">
                                <div style="height:80px;padding-top:20px">
                                    <RadioGroup v-model="theme.deleteTargetOrPeriodModalId">
                                        <Radio style="display:block;font-size: 16px;font-weight: 500;" label="1"
                                               value="1">删除当前期数
                                        </Radio>
                                        <Radio style="display:block;font-size: 16px;font-weight: 500;" label="0"
                                               value="0">删除当前主题
                                        </Radio>
                                    </RadioGroup>
                                </div>
                ...
                if (step === 0) {
                    ...
                    if (flag) {
                        ...
                        if (this.target.totalTime % this.period.availableTime === 0) {
                            ...
                            days = this.target.totalTime / this.period.availableTime + 1;
                        }
                        let date = new Date();
                        date.setTime(date.getTime() + 24 * 60 * 60 * 1000 * days);
                        this.period.deadline = date.getFullYear() + "-" + (date.getMonth() + 1) + "-" + date.getDate();
                    }
                }
            },
            resetTarget() {
